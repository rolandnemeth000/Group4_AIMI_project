...
TEST_TABULAR_PATH = Path("/home/rolandnemeth/AIMI_project/repos/picai_unet_semi_supervised_gc_algorithm/test/clinical-information-prostate-mri.json")
...
def main():
    ...
    tabular_data_dict = {"patient_age": age, "psa": psa, "psad": psad, "prostate_volume": prostate_volume}

    tabular_data_df = pd.DataFrame(tabular_data_dict, index=[0]) if isinstance(age, (int, float)) else pd.DataFrame.from_dict(tabular_data_dict)
