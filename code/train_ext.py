...
from torch.optim import Adam, AdamW, SGD, SparseAdam
...
def main():
    ...
    parser.add_argument('--optimizer', type=str, default='Adam', help='Optimizer for training. Options: Adam, AdamW, SparseAdam, SGD')
    ...
    for f in args.folds:
        ...
        match args.optimizer:
            case 'AdamW':
                optimizer = AdamW(params=model.parameters(), lr=args.base_lr, amsgrad=True)
            case 'SGD':
                optimizer = SGD(params=model.parameters(), lr=args.base_lr, amsgrad=True)
            case 'SparseAdam':
                optimizer = SparseAdam(params=model.parameters(), lr=args.base_lr, amsgrad=True)
            case _:
                optimizer = torch.optim.Adam(params=model.parameters(), lr=args.base_lr, amsgrad=True)
